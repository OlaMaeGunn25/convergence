...
def is_chrome_running():
    try:
        # Check if chrome.exe is in process list (exclude our own antigravity browser profile if possible, or just check general chrome)
        output = subprocess.check_output('tasklist /FI "IMAGENAME eq chrome.exe"', shell=True).decode('latin1')
...
def extract_profile_cookies(profile_path, profile_name, aes_key):
    ...
    # Direct copy since Chrome is closed now
...
def main():
    ...
    print("[*] Checking for active Chrome processes...")
    if is_chrome_running():
        print("[!] Google Chrome is currently running and locking the cookie databases.")
        print("[!] PLEASE CLOSE ALL CHROME WINDOWS now to allow extraction.")
        print("[*] Waiting for Chrome to exit...")
        while is_chrome_running():
            time.sleep(1)
            sys.stdout.write(".")
            sys.stdout.flush()
        print("\n[+] Chrome closed! Starting cookie extraction...")
    else:
        print("[+] Chrome is not running. Starting extraction immediately...")
    ...
    print("\n[+] Done! You can now reopen Chrome.")
