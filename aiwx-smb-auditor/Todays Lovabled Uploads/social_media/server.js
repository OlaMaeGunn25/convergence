...
if (missingEnv.length) {
  ...
  console.error('[BOOT] Server cannot start safely. Check your .env file.');
  process.exit(1);
}
...
app.use((req, res, next) => {
  ...
  if (!origin || ALLOWED_ORIGINS.includes(origin) || origin === 'null' || origin.includes('localhost') || origin.includes('127.0.0.1') || origin.includes('lovable')) {
    if (origin) {
...
// Rate limiting — protect expensive/scouting endpoints
...
app.use('/logs', express.static(path.resolve(__dirname, '../aiwx-social-media-agent/logs')));
...
app.get('/api/analytics-config', (req, res) => {
  res.json({
    success: true,
    measurementId: process.env.GA4_MEASUREMENT_ID || 'G-V2Z3W6F8G2'
  });
});
...
app.post('/api/publish-post', (req, res) => {
  ...
  // Resolve agent directory and run script using execFile to handle multiline text and quotes safely on Windows
  const agentDir = path.resolve(__dirname, '../aiwx-social-media-agent');
  
  const args = ['publish_api.js', '--platform', platform, '--text', text];
  if (image) {
 ...
 */
const SCHEDULE_FILE = path.resolve(__dirname, '../aiwx-social-media-agent/config/campaign_schedule.json');
...
function sendNotification(title, message) {
  ...
  execFile('powershell.exe', ['-ExecutionPolicy', 'Bypass', '-File', 'send_notification.ps1', title, message], { cwd: AGENT_DIR }, (err) => {
    if (err) console.error('[Server] Failed to trigger notification:', err);
  });
}
...
const ALERTS_FILE = path.resolve(__dirname, '../aiwx-social-media-agent/config/activity_alerts.json');
...
setInterval(() => {
  ...
  if (isTimeWindow && lastProspectRunDate !== todayDateStr) {
    ...
    const targetDir = 'c:/Users/dahao/.gemini/antigravity/scratch/aiwx-social-media-agent';
    const cmd = `node prospecting_agent.js --platform linkedin --vertical "${targetVertical}"`;
    
    exec(cmd, { cwd: targetDir }, (error, stdout, stderr) => {
      if (error) {
        console.error(`[-] Prospecting run failed: ${error.message}`);
        return;
      }
      console.log(`[+] Prospecting run completed successfully:\n${stdout}`);
...
app.post('/api/run-prospecting', (req, res) => {
  ...
  const targetDir = 'c:/Users/dahao/.gemini/antigravity/scratch/aiwx-social-media-agent';
  const cmd = `node prospecting_agent.js --platform ${targetPlatform} --vertical "${targetVertical}" ${isDryRun ? '--dry-run' : ''}`;
  
  exec(cmd, { cwd: targetDir }, (error, stdout, stderr) => {
    if (error) {
      console.error(`[-] Manual prospecting failed: ${error.message}`);
      return res.status(500).json({ success: false, error: error.message });
    }
    console.log(`[+] Manual prospecting completed successfully:\n${stdout}`);
...
app.get('/api/posts-library', (req, res) => {
  const libraryPath = path.resolve(__dirname, '../aiwx-social-media-agent/social_media_posts_library.md');
  if (!fs.existsSync(libraryPath)) {
...
const OUTREACH_REGISTRY = path.resolve(__dirname, '../aiwx-social-media-agent/config/prospects_outreach.json');
...
const SUPABASE_CREDENTIALS = path.resolve(__dirname, '../aiwx-social-media-agent/config/supabase_credentials.json');

app.get('/api/supabase-credentials', (req, res) => {
  if (fs.existsSync(SUPABASE_CREDENTIALS)) {
    try {
      const creds = JSON.parse(fs.readFileSync(SUPABASE_CREDENTIALS, 'utf8'));
      res.json({
        success: true,
        supabaseUrl: creds.supabaseUrl,
        hasKey: !!creds.serviceRoleKey
      });
    } catch (e) {
      res.json({ success: false, error: 'Failed to parse credentials.' });
    }
  } else {
    res.json({ success: true, supabaseUrl: '', hasKey: false });
  }
});

app.post('/api/supabase-credentials', (req, res) => {
  const { supabaseUrl, serviceRoleKey, supabaseKey } = req.body;
  const keyToSave = serviceRoleKey || supabaseKey;
  if (!supabaseUrl || !keyToSave) {
    return res.status(400).json({ success: false, error: 'URL and Service Role Key are required.' });
  }

  try {
    const dir = path.dirname(SUPABASE_CREDENTIALS);
    if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
    
    fs.writeFileSync(SUPABASE_CREDENTIALS, JSON.stringify({ supabaseUrl, serviceRoleKey: keyToSave }, null, 2), 'utf8');
    res.json({ success: true });
  } catch (err) {
    res.status(500).json({ success: false, error: 'Failed to write credentials.' });
  }
});
...
app.post('/api/export-crm', async (req, res) => {
  ...
  if (!fs.existsSync(SUPABASE_CREDENTIALS)) {
    return res.status(400).json({ success: false, error: 'Supabase credentials not configured. Please save them in the settings first.' });
  }

  let creds;
  try {
    creds = JSON.parse(fs.readFileSync(SUPABASE_CREDENTIALS, 'utf8'));
  } catch (e) {
    return res.status(400).json({ success: false, error: 'Failed to read Supabase credentials.' });
  }

  const { supabaseUrl, serviceRoleKey } = creds;
  if (!supabaseUrl || !serviceRoleKey) {
    return res.status(400).json({ success: false, error: 'Supabase URL and Service Role Key are required.' });
  }
  ...
  try {
    ...
    // Helper to send HTTPS requests to Supabase REST endpoints
    const postTable = (table, payload) => {
        ...
        const req = https.request(options, (res) => {
          ...
          res.on('end', () => {
            if (res.statusCode >= 200 && res.statusCode < 300) {
              ...
              reject(new Error(`Supabase REST ${table} returned status ${res.statusCode}: ${data}`));
            }
          });
        });
