...
const MOCK_RSS_XML_STRING = `<?xml version="1.0" encoding="UTF-8"?>
<rss version="2.0">
  <channel>
    <item>
      ...
      <description>New Gartner research...</description>
      <category>Leadership &amp; Growth Strategy</category>
      <category>AI Without Hype</category>
    </item>
    <item>
      <title>Human-in-the-Loop ROI</title>
      <link>https://convergence-ai.com/blog/human-in-the-loop-roi</link>
      <pubDate>Thu, 16 Apr 2026 14:00:00 GMT</pubDate>
      <description>The AI rollouts...</description>
...
global.fetch = function(url, options) {
    if (url.includes('/api/hitl/action')) {
        return Promise.resolve({
            ok: true,
            json: () => Promise.resolve({
                success: true,
                hitlQueue: global.STATE.hitlQueue,
                completedCount: global.STATE.completedCount
            })
        });
    }
    if (url.includes('/blog-rss')) {
        return Promise.resolve({
            ok: true,
            text: () => Promise.resolve(MOCK_RSS_XML_STRING)
        });
    }
    return Promise.resolve({
        ok: true,
